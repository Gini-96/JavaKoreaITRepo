...
/*
	GSON
	- Google 에서 만든 JSON Java library
	- 자바객체 <-> JSON 변환
	- JSON <-> 자바객체 변환
...
public class GsonTest1 {
	...
	public static void main(String[] args) {
		...
		// PrettyPrinting : JSON 문자열을 들여쓰기 해서 정렬해서 보여줌
		...
		// JSON 객체는 JSON 문자열을 가진 객체, JSON 문자열은 JSON 객체 형태의 문자열
		String jsonStr = gson3.toJson(jsonObject); // JSON 객체 -> JSON 문자열
		System.out.println(jsonStr);
		
		// JAVA 객체를 JSON 문자열로 변환
		// 프로그램에서 네트워크를 통해서 JAVA 객체를 JSON 문자열로 변환해서 내보낼때 사용
		Person person = new Person("홍길동",30);
		String personJsonStr = gson3.toJson(person); // JAVA 객체 -> JSON 문자열
...
//		System.out.println(jsonStr2);
		...
		Person person2 = gson3.fromJson(jsonStr2, Person.class);// JSON 문자열 -> JAVA 객체
		...
		map.put("age", "20");
		map.put("gender", "여성");
		String mapStr = gson3.toJson(map); // Map 객체를 JSON 문자열로 변환
		System.out.println(mapStr);
		
		// JSON 문자열을 Map 객체로 변환
		String jsonStr3 = "{\"name\":\"강감찬\",\"age\":\"30\",\"gender\":\"남성\"}";
		...
		// TypeToken 을 사용하여 List<Person>의 타입을 명시적으로 정의
		List<Person> personList2 = gson3.fromJson(jsonStr4, ArrayList.class);
